...
DFL168A::DFL168A() : J1939(250000,this), J1708(this),ISO15765(this),GPS(9600,500,this) {
  /*OnewireID[0]=0;
  OnewireID[1]=0;
  OnewireID[2]=0;
  OnewireID[3]=0;
...
DFL168A::DFL168A(HardwareSerial * XX, byte currProtocol, int Timeout,long J1939Baudrate, long DEV_Baudrate, int DEV_timeout): J1939(J1939Baudrate,this),J1708(this),ISO15765(this),GPS(DEV_Baudrate,DEV_timeout,this) {
 ...
 byte DFL168A::ATCommand(const char * CMD)
 {
	...
	if (WAITING != ReturnValue) { tick4timeout = millis(); mySerial->println(CMD); index_MyReceiveData = 0;}
    //if (0==mySerial->available()) {ReturnValue=WAITING; return WAITING;}   
	if (0 == mySerial->available())
	{
		//if (millis() - tick4timeout<mySerial->TimeOut)
		if (millis() - tick4timeout<1000)
		{
			...
			ReturnValue = WAITING; return WAITING;
		}
		else
		{
			if (index_MyReceiveData > 1)
			...
			ReturnValue = FAIL; return FAIL;
		}
	}
	else
	{
		//has char received
		MyReceiveData[index_MyReceiveData++] = mySerial->read();
		ReturnValue = WAITING;
		return WAITING;
	}
...
GotResponseJudge:
	if ('>' == MyReceiveData[index_MyReceiveData - 1])
	{
		...
		ReturnStr = String(MyReceiveData);
		index_MyReceiveData = 0;  //for next time use
	}
	else
	{
...
byte DFL168A::DigitalCommand(const char * CMD)
{
   ...
   if ((J1939_PROTOCOL==currentProtocol)||(J1708_PROTOCOL==currentProtocol))  
   {
      if (WAITING!=ReturnValue) { 
		  ...
		  tick4timeout = millis(); mySerial->println(CMD); index_MyReceiveData = 0;	  }
   }
   else
   {
      ...
      if ((millis()-tick)>149) //142,bad, 146ok
      {
         if (!Sent) { 
			 Sent=true; 
			 //Serial.print("Digital Cmd ");  Serial.print(CMD); Serial.println( " start, response is:"); 
			 tick4timeout=millis(); clearRXDBuffer(); mySerial->println(CMD); index_MyReceiveData = 0;}
      }
	  else
	  {
		  return WAITING;
   ...
   if (mySerial->available()<=3)
   {
     ...
     else
     {
        //time out
        while (mySerial->available()) mySerial->read();  //clear buffer
		Sent = false;  //version 1.1.1 add: Jan 22/2021
        ReturnValue=FAIL;
        return FAIL;
     }
...
byte DFL168A::getDIN(int portNo, bool & Value)
{
  byte Ret;
  String TempStr;
  char bufferx[15];
  ...
  return LOW;
  #else     //version 1.1.1 : Jan 21/2021
  Ret = ATCommand(bufferx);
  if (WAITING == Ret)  return Ret; 
  ReturnStr.trim();
  while (mySerial->available()) mySerial->read();
  if (ReturnStr == "1") { Value = true; return SUCCESS; }
  if (ReturnStr == "0") { Value = false; return SUCCESS; }
  return FAIL;
  #endif
}

byte DFL168A::setDOUT(int portNo, bool Value)
{
  byte Ret;
  String TempStr;
  char bufferx[15];
...
byte DFL168A::getAnalog(float & ADValue)
{
  #if 0
  ...
  while (mySerial->available()) mySerial->read();
  ReturnStr.trim();
  ADValue = ReturnStr.toFloat();  //0.0 to 999.000
  return SUCCESS;
  #endif
